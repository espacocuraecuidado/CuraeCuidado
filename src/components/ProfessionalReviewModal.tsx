      ...
      >
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-2">
            <Button variant={aboutSite ? "outline" : "default"} className="h-12 gap-2" onClick={() => setAboutSite(false)}>
              <MessageCircle className="h-4 w-4" /> Profissional
            </Button>
            <Button variant={!aboutSite ? "outline" : "default"} className="h-12 gap-2" onClick={() => setAboutSite(true)}>
              <Star className="h-4 w-4" /> Sobre site
            </Button>
          </div>
          <div>
          ...
          <Textarea
            placeholder={aboutSite ? "O que achou do site, entrega, atendimento?" : "Conte como foi sua experiência com o profissional"}
            value={comment}
            onChange={(e) => setComment(e.target.value)}
            rows={4}
          />
          <Button className="w-full h-12 text-lg" onClick={submitReview} disabled={!rating || loading}>
